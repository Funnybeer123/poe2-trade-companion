...
(async () => {
  ...
  const report = setInterval(() => {
    ...
    console.log(`${((Date.now() - began) / 1000).toFixed(1).padStart(5)} s  ${(s.decision?.kind ?? "-").padEnd(5)} d=${String(s.observation?.offset?.distance ?? "-").padStart(5)} conf=${s.observation?.confidence ?? "-"} odo=${s.odometry ? `${s.odometry.tracked ? "ok" : "no"}:${s.odometry.quality}:${s.odometry.via}:${s.odometry.trailPoints}` : "-"} moved=${s.odometry?.movedPxPerSec ?? "-"}px/s/${s.odometry?.movedTracked ?? "-"}trk life=${flask ? `${flask.status.life ?? "?"}%/${flask.status.presses}` : "-"} sprint=${s.sprinting ? "ON" : "off"}/${s.stats?.sprints ?? 0} plan=${s.terrain ? `${s.terrain.planned ? s.terrain.pathPx : "none"}/${s.terrain.walls}w/${s.terrain.bumps}b${s.terrain.blockedAhead ? "!" : ""}${s.terrain.wallBetween ? (s.terrain.reachesLeader ? ((s.terrain.openShare ?? 0) > 0.25 ? `=VOID${Math.round((s.terrain.openShare ?? 0) * 100)}%` : "=ROUTE") : "=wall") : ""}` : "-"} origin=${s.observation?.originVerified ? "ok" : "NO"}(${s.observation?.evidence.originScore ?? "-"}) ${s.observation?.evidence.searched ?? ""} obs/s=${s.stats?.observationsPerSecond ?? "-"} cycle p50/p95=${s.stats?.cycleMsP50 ?? "-"}/${s.stats?.cycleMsP95 ?? "-"} clicks=${s.stats?.clicks ?? 0} previewed=${s.stats?.previewed ?? 0} loot=${s.stats?.lootClicks ?? 0}/${s.stats?.lootLabels ?? 0}labels/${s.stats?.lootScans ?? 0}scans refused=${s.stats?.refused ?? 0} manual=${s.stats?.manualTakeovers ?? 0} input p50/p95=${s.stats?.captureToInputMsP50 ?? "-"}/${s.stats?.captureToInputMsP95 ?? "-"} lag=${worstLag}ms/${stalls}stalls gc=${Math.round(worstGc)}ms/${Math.round(gcMs)}total mem=${(() => { const m = process.memoryUsage(); const mb = (n: number) => Math.round(n / 1048576); return `rss${mb(m.rss)}/heap${mb(m.heapUsed)}/ext${mb(m.external)}/ab${mb(m.arrayBuffers)}`; })()}MB  ${s.reason}`);
