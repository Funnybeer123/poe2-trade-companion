...
foreach ($case in @(@(128,123,157,255), @(150,150,200,255), @(170,170,187,0), @(120,120,138,255), @(62,127,165,0), @(148,140,138,0), @(180,120,200,0), @(90,90,100,0), @(240,240,250,0), @(238,180,97,0))) { if ([FollowWin]::ChannelValue($case[0], $case[1], $case[2], 'walls') -ne $case[3]) { throw "Walls channel mismatch for $($case[0..2] -join ',')" } }
