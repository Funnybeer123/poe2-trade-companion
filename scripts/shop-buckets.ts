...
/** The Review tab's label from the triage export (the sorter's own routing). */
function reviewTabLabel(): string {
  const file = path.join(outDir, "triage.json");
  if (!existsSync(file)) return DEFAULT_TRIAGE_ROUTING.reviewTab;
  try {
    const parsed = JSON.parse(readFileSync(file, "utf8")) as { routing?: Partial<TriageRouting> };
    const label = parsed.routing?.reviewTab?.trim();
    return label || DEFAULT_TRIAGE_ROUTING.reviewTab;
  } catch {
    return DEFAULT_TRIAGE_ROUTING.reviewTab;
  }
}
...
try {
  ...
  if (reviewSource) {
    const reviewTab = reviewTabLabel();
