...
export class FollowerInputSink implements InputSink {
  ...
  async emit(action: InputAction): Promise<void> {
    ...
    const result = await this.host.send({ op: "moveclick", x: action.x, y: action.y, expectedHwnd: frame.hwnd, viewWidth: frame.viewWidth, viewHeight: frame.viewHeight, capturedAtQpcMs: frame.capturedAtQpcMs, maxAgeMs: this.maxAgeMs });
