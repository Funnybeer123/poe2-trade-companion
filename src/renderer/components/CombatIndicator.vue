...
<template>
  <div v-if="state?.running" class="combat-indicator">
    <RouterLink to="/tools/combat">{{ state.config.sigilSequence.enabled ? `Sigil macro armed · press ${state.config.unleash.key}` : state.config.dryRun ? 'Combat preview' : 'Combat running' }} · F8</RouterLink>
